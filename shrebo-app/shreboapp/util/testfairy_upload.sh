#!/bin/sh
# source https://raw.githubusercontent.com/testfairy/command-line-uploader/master/testfairy-upload.sh

UPLOADER_VERSION=1.09

# Put your TestFairy API_KEY here. Find it in your TestFairy account settings.
TESTFAIRY_API_KEY=c870ae1503c441b953a4de834733fcbbe3e36270
...
usage() {
	echo "Usage: testfairy-upload.sh path/to/apk path/to/keystore path/to/storepass path/to/alias"
	echo
}
...
verify_tools() {
	...
	oOUTPUT=$( ${KEYTOOL} -help 2>&1 | grep "keypasswd" )
	if [ $? -ne 0 ]; then
